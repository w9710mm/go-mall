...
var redishelper, redisContext = dao.GetRedis()
...
func (s umsMemberService) GetAuthCode(telephone string) (int, error) {
	...
	_, err := redishelper.Set(redisContext, redis_prefix_authcode+telephone, vcode, time.Duration(redis_expire_authcode)*time.Second).Result()
	if err != nil {
		return 0, err
	}
...
func (s umsMemberService) VerifyAuthCode(telephone string, authCode string) (bool, error) {
	realAuthCode, err := redishelper.Get(redisContext, redis_prefix_authcode+telephone).Result()
	if err != nil {
		return false, err
	}
